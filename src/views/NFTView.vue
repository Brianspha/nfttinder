  ...
  <v-container fluid>
    <v-row v-if="isLoading" class="loading" justify="center" align="center">
     ...
     <v-row v-else class="loading-nothing" justify="center" align="center">
      <div class="loading-icon-nothing">Nothing Yet:( try minting a new token under the profile tab</div>
    </v-row>
  ...
  methods: {
            ...
            keys.map(async (key, index) => {
              ...
              axios.get(tokenURI).then((nftDetails) => {
                ...
                if (nftDetails.status === 200) {
                  ...
                  nftDetails.value = new bigNumber(etherConverter(details[4],"wei","eth")).toNumber(
                    2
                  );
    ...
    setApproval(approval) {
      ...
      */
      ...
      console.log("approvedCards: ", cards.data[cards.index]);
      this.$store.state.selectedNFTS.push(cards.data[cards.index]);
      cards.index++;
