...
contract BidContract is Ownable, ReentrancyGuard {
    ...
    function widthDrawTokenFromEscrow(uint256 tokenId) public nonReentrant{ //@dev figure out how to restrict calling this function whilst a bid is active 
        require(tokenId> 0, "Invalid token Id");
        require(msg.sender != address(0), "Invalid sender address");
    ...
    function getBid(uint256 bidId) public view returns (address,uint256,uint256,uint256,uint256,uint256,bool)
    {
        return (nftBids[bidId].owner,nftBids[bidId].startTime,nftBids[bidId].endTime,nftBids[bidId].startingPrice,nftBids[bidId].currentMax,nftBids[bidId].tokenId,nftBids[bidId].active);
    }
